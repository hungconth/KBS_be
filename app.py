...
import random
import json
from hung import solve
from hung import POS_dict
import uvicorn
...
async def generate_sentence(request: SentenceRequest):
    words = request.words
    # Call the existing logic to process the words and generate a sentence
    # For example:
    print("words:",words)
    result = solve(words)
...
@app.post("/check-pos/")
async def check_pos(request: WordRequest):
    word = request.word
    # Check if the word is in POS_dict and return the response
    if word in POS_dict or (word[0].isupper() if word else False):
        if (word[0].isupper() if word else False):
            POS_dict[word] = ['Np']
        return {"in_dict": True, "response": POS_dict[word]}
    else:
        
        return {"in_dict": False, "response": "Word not found in POS dictionary"}
